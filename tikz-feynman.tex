...
\begin{titlepage}
  ...
  \begin{center}
    ...
    \vspace{0.7em}
    {Version \texttt{\@version} \qquad \today}
...
\href{http://tex.stackexchange.com}{tex.se} which initially inspired me to write
this package.
...
\href{https://www.ctan.org}{\textsc{ctan}} as it is just a personal project of
mine; however, if enough people find it useful, I will look into making it
available through \textsc{ctan}.

To install this package, download the latest version from
\url{https://github.com/JP-Ellis/tikz-feynman/releases} and place it in your
|texmf| tree.  On Linux, you can find where the location of your user |texmf|
tree with |kpsewhich -var-value=TEXMFHOME| which is by default set to |~/texmf|.
For Windows and OS X users, there should be something analogous.  The |texmf|
directory expects a specific directory structure, so you will need to extract
the \tikzfeynmanname{} files into |<TEXMF>/tex/latex/tikz-feynman|.

In v3.0.0 and v3.0.1 of \tikzname, there is a bug in the Lua component of the
graphdrawing library which prevents it from handling coordinate nodes properly.
As a result, the automatic placement of nodes does not work unless the following
patch is applied.  This bug does not affect other parts of this package, so you
can still use it for the pre-defined styles if you wish. \\
...
After installing the package and applying the patch (if needed), the
\tikzfeynmanname{} package can be loaded with |\usepackage{tikz-feynman}| in the
preamble.
...
(|;|) to finish the environment.  For example, a simple \(t\)-channel diagram
is:
...
|particle=|\meta{text}.  As an example, we take the generic \(t\)-channel
...
\begin{tikzpicture}
  \begin{feynman}
    ...
    \vertex[right=2cm of a2] (a3) {\(\overline u\)};

    \vertex[below=0cm of a1] (b1) {\(d\)};
    \vertex[below=0cm of a3] (b2) {\(d\)};

    \vertex[above=0.5cm of a3] (c1) {\(u\)};
    \vertex[above=0cm of c1] (c3) {\(\overline d\)};
    ...
    \draw [decoration={brace}, decorate] (a3.north east) -- (b2.south east)
          node [pos=0.5, right] {\(\pi^{-}\)};
  \end{feynman}
\end{tikzpicture}
\end{codeexample}
...
\begin{tikzpicture}
  \begin{feynman}
    ...
    \vertex[right=3cm of b4] (b5) {\(\overline d\)};

    \vertex[above=0.5cm of a6] (c1) {\(\overline u\)};
    \vertex[above=0cm of c1] (c3) {\(d\)};
    \vertex at ($(c1)!0.5!(c3) - (1cm, 0)$) (c2);
...
\begin{tikzpicture}
  \begin{feynman}
    ...
    \vertex[below=1.5cm of a1] (b1) {\(d\)};
    \vertex[below=1.5cm of a5] (b2) {\(d\)};

    \vertex[below=0cm of a5] (c1) {\(s\)};
    \vertex[above=0cm of b2] (c3) {\(\overline s\)};
    \vertex at ($(c1)!0.5!(c3) - (1cm, 0)$) (c2);
