...
\tikzfeynmanset{
  compat/.estore in=\tikzfeynman@compat,
  compat=\tikzfeynman@empty,
}

\AtBeginDocument{
  \ifx\tikzfeynman@version\tikzfeynman@compat\else
    \ifx\tikzfeynman@compat\tikzfeynman@empty
      \PackageWarning{tikz-feynman}{Consider adding \string\tikzfeynmanset{compat=\tikzfeynman@version} to your preamble.}
    \else
      \PackageWarning{tikz-feynman}{The preamble requested version \tikzfeynman@compat, but the system version is \tikzfeynman@version.  This may lead to some changes in the output.}
    \fi
  \fi
}
...
\def\tikzfeynman@apply@patch{
  \directlua{dofile("tikzfeynman.patch.lua")}
}
