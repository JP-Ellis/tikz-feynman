...
\def\tikzfeynman@search@also@tikz{%
  % make sure the search path is set.
  \pgfkeysifdefined{/tikzfeynman/.unknown/.@cmd}%
    {\relax}%
    {\pgfkeysalso{/tikzfeynman/.search also=/tikz}}%
}%
...
\pgfdeclaredecoration{old complete sines}{initial}{
  %% Begin by computing the overall path length and find how many sine waves we
  %% can fit in.  Nothing happens beyond the computation.
  \state{initial}[
    width=0pt,
    ...
    persistent precomputation={
      ...
      \pgfmathsetmacro\tikzfeynman@matching@length{
        \pgfdecoratedinputsegmentlength / int(\pgfdecoratedinputsegmentlength/\pgfdecorationsegmentlength)}
      \setlength{\pgfdecorationsegmentlength}{\tikzfeynman@matching@length pt}
      },
  ]{}
  %% Start drawing the sine waves
  \state{sine}[
  ...
  %% Final state (nothing special happens)
  \state{final}{
    \pgfkeys{/pgf/decoration/amplitude=0pt}
  }
}
